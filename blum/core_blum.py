...
import blum.config_blum as config_blum
...
class AutoClicker:
    ...
    def check_and_click_play_button(self, sct, monitor):
        ...
        if current_time - self.last_check_time >= random.uniform(config_blum.CHECK_INTERVAL_MIN, config_blum.CHECK_INTERVAL_MAX):
            self.last_check_time = current_time
            templates = [
                cv2.imread(os.path.join("template_play_button.png"), cv2.IMREAD_GRAYSCALE),
                cv2.imread(os.path.join("template_play_button1.png"), cv2.IMREAD_GRAYSCALE)
            ]
    ...
    def tickets_out(self):
        tickets_image = r"tickets_out.png"
        try:
            button_location = pyautogui.locateOnScreen(tickets_image, confidence=0.8)
            if button_location:  # If the image is found, exit the loop
                self.logger.log(f"the games are over - I'm moving on to the next account")
            return True
        except:
            return False
    ...
    def click_color_areas(self):
        ...
        # with mss.mss() as sct:
        #     keyboard.add_hotkey(config_blum.HOTKEY, self.toggle_script)
        #     self.logger.log(f'Нажмите {config_blum.HOTKEY} для запуска/остановки скрипта.')
        with mss.mss() as sct:
            while True:
                # if self.running:
    ...
    def check_and_click_freeze_button(self, sct, monitor):
        freeze_hsvs = [self.hex_to_hsv(color) for color in config_blum.FREEZE_COLORS_HEX]
        current_time = time.time()
        if current_time - self.last_freeze_check_time >= 1 and current_time >= self.freeze_cooldown_time:
            ...
            for freeze_hsv in freeze_hsvs:
                ...
                for contour in contours:
                    ...
                    right_bottom_x = monitor["width"] - config_blum.OFFSET_X
                    right_bottom_y = monitor["height"] - config_blum.OFFSET_Y
    ...
    def random_delay_before_restart(self):
        delay = random.uniform(config_blum.CHECK_INTERVAL_MIN, config_blum.CHECK_INTERVAL_MAX)
...
def blum_main():
    ...
    windows = list_windows_by_title(config_blum.KEYWORDS)

    # Инициализация переменной choice
    choice = None

    # Проверяем, есть ли окно с заголовком "TelegramDesktop"
    for i, (title, hwnd) in enumerate(windows):
        if title == "TelegramDesktop":
            choice = i
            break
    ...
    while True:
        try:
            target_percentage = str(config_blum.target_percentage)
    ...
    while True:
        try:
            collect_freeze = config_blum.collect_freeze
    ...
    logger = Logger("[https://t.me/cucumber_scripts]")
    logger.log("Tickets have been found. Launching games")

    auto_clicker = AutoClicker(hwnd, config_blum.TARGET_COLORS_HEX, config_blum.NEARBY_COLORS_HEX, config_blum.THRESHOLD, logger, target_percentage, collect_freeze)
    try:
